...
function onClick()
	local Part1 = game.Workspace.TechnicaPannel:WaitForChild("MainPannel").P1.Buttons.Presets.Row2.Preset6.LC
	bool = not bool
	if bool then
		for i,v in pairs(game.Workspace.TaLights:GetChildren()) do spawn(function()
				if v.Group_Values.Group.Value == 1 then
					script.Preset_6_Effect_1.Disabled = false
				end
			end)
		end
		...
		for i,v in pairs(game.Workspace.TaLights:GetChildren()) do spawn(function()
				if v.Group_Values.Group.Value == 1 then
					script.Preset_6_Effect_1.Disabled = true
...
game.Workspace.TechnicaPannel:WaitForChild("MainPannel").P1.Buttons.Presets.Row2.Preset6.MouseButton1Click:Connect(onClick)

function onClick2()
	local Part1 = game.Workspace.TechnicaPannel:WaitForChild("MainPannel").P1.Buttons.Presets.Row2.Preset6.RC
	bool = not bool
	if bool then
		for i,v in pairs(game.Workspace.TaLights:GetChildren()) do spawn(function()
				if v.Group_Values.Group.Value == 2 then
					script.Preset_6_Effect_2.Disabled = false
				end
			end)
		end
		...
		for i,v in pairs(game.Workspace.TaLights:GetChildren()) do spawn(function()
				if v.Group_Values.Group.Value == 2 then
					script.Preset_6_Effect_2.Disabled = true
					v.Motors.Tilt.DesiredAngle = math.rad(45)
					v.Head.Beam.SpotLight.Brightness = 0
					v.Head.Lens.Transparency = 1
					v.Head.Beam.light.Transparency = NumberSequence.new(1)
...
game.Workspace.TechnicaPannel:WaitForChild("MainPannel").P1.Buttons.Presets.Row2.Preset6.MouseButton2Click:Connect(onClick2)
